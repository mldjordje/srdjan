...
export default function AdminCalendarPage() {
  const requestedWorkerId =
    typeof window === "undefined"
      ? ""
      : (new URLSearchParams(window.location.search).get("workerId") || "").trim();
  ...
  const selectedService = useMemo(
    ...
    [appointmentForm.serviceId, serviceItems]
  );
  ...
  const activeWorker = workers.find((worker) => worker.id === workerId) || null;
  ...
  const mapAppointment = (row: AppointmentRow): Appointment => ({
    id: row.id,
    clientName: row.clients?.full_name || "Klijent",
    phone: row.clients?.phone || "",
    email: row.clients?.email || "",
    serviceId: row.service_id,
    serviceName: row.service_name_snapshot,
    duration: `${row.duration_min_snapshot} min`,
    price: row.price_snapshot,
    date: row.date,
    time: normalizeTimeInput(row.start_time),
    notes: row.note || "",
  ...
  const mapBlock = (row: BlockRow): Block => ({
    id: row.id,
    date: row.date,
    time: normalizeTimeInput(row.start_time),
  ...
  const fetchAppointments = async (date: string) => {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data?.error || "Ne mogu da preuzmem termine.");
    }

    const rows = (Array.isArray(data.appointments) ? data.appointments : []) as AppointmentRow[];
  ...
  const fetchBlocks = async (date: string) => {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data?.error || t.cannotLoadBlocks);
    }

    const rows = (Array.isArray(data.blocks) ? data.blocks : []) as BlockRow[];
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const items = (Array.isArray(data.clients) ? data.clients : []).map(
        (client: { id: string; full_name: string; phone: string; email?: string }) => ({
          id: client.id,
  ...
  const fetchServiceItems = async () => {
    ...
    try {
      ...
      const items = (Array.isArray(data.services) ? data.services : [])
        ...
        .map((item: ServiceRow) => ({
          id: item.services?.id || item.id,
          name: item.services?.name || "Usluga",
  ...
  useEffect(() => {
    if (weekDateStrings.length === 0 || !workerId) {
      return;
    }
    refreshData(weekDateStrings);
  }, [weekDateStrings, workerId]);
  ...
  const handleDeleteAppointment = async (appointment: Appointment) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.error || t.cannotDeleteAppointment);
      }
  ...
  const handleUpdateAppointmentStatus = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.error || t.cannotSaveStatus);
      }
  ...
  const handleCreateBlock = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!locationId || !workerId) {
      setStatus({ type: "error", message: "Prvo izaberi radnika." });
      return;
    }
    ...
    try {
      const endpoint = "/api/admin/blocks";
      ...
      if (!createResponse.ok) {
        throw new Error(data?.error || t.cannotSaveBlock);
      }
  ...
  const handleSaveAppointment = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.error || t.cannotSaveAppointment);
      }
  ...
  const handleDeleteBlock = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/blocks?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data?.error || t.cannotDeleteBlock);
      }
    ...
    >
      <div className="admin-grid">
        <div className="calendar-toolbar">
          ...
          <div className="calendar-toolbar__actions">
            ...
            <button
              className="button outline small"
              type="button"
              onClick={() => refreshData(weekDateStrings)}
              disabled={!workerId}
            >
              Osvezi
            </button>
        ...
        <div className="admin-card">
          ...
          <p>
            Aktivni radnik za prikaz kalendara:{" "}
            <strong>{activeWorker?.name || "nije izabran"}</strong>
          </p>
            ...
            {workers.map((worker) => (
              <button
                ...
                onClick={() => {
                  ...
                  setLocationId(worker.location_id);
                  setSelectedAppointment(null);
                  setEditingAppointment(null);
                  setIsSlotModalOpen(false);
                }}
